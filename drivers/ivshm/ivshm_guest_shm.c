...
static int probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (idev->fact->signature != GUEST_SHM_SIGNATURE) {
		dev_err(&pdev->dev, "Signature incorrect. %llx != %llx",
			(unsigned long long)GUEST_SHM_SIGNATURE, (unsigned long long) idev->fact->signature);
		return -ENXIO;
	}

	if (idev->fact->status != GSS_OK) {
		dev_err(&pdev->dev, "creating failed: %d", idev->fact->status);
		return -idev->fact->status;
	}
	...
	dev_info(&pdev->dev, "virtio%ld creation size %x, paddr=%llx", idx, idev->fact->size, (unsigned long long)paddr);
	...
	idev->info.dev_mmio_len = idev->fact->size * 0x1000 - ((idev->info.dev_mmio - (GUEST_SHM_PADDR + (idx << 32)) - 0x1000 + (min_align - 1)) & ~(min_align - 1));
	dev_info(&pdev->dev, "min_align: 0x%lx, mmio: 0x%llx, mmio_len: 0x%llx", min_align, idev->info.dev_mmio, idev->info.dev_mmio_len);

	err = ivshm_register_region(idev->info.ivshm_dev, "default", idev->info.dev_mmio, idev->info.dev_mmio_len, 1 /* use INTx */, &iregion);
	if (err) {
