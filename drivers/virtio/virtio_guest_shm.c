...
static irqreturn_t virtio_guest_shm_early_irq_handler(struct virtio_shmem_device *vi_dev)
{
	...
	for (i = 0; i < GUEST_SHM_MAX_CLIENTS; ++i) {
		if (attach_list_changes & (1u << i)) {
			dev_info(&vi_dev->pci_dev->dev, "client %2u %stached", i, (attach_list & (1u << i)) ? "at" : "de");
		}
	}
				...
				const struct pci_device_id *pci_id)
{
	...
	if (vi_data->fact->signature != GUEST_SHM_SIGNATURE) {
		dev_err(&pci_dev->dev, "Signature incorrect. %llx != %llx",
			(unsigned long long)GUEST_SHM_SIGNATURE, (unsigned long long) vi_data->fact->signature);
		ret = -ENXIO;
		goto err_enable;
	}
	...
	dev_info(&pci_dev->dev, "virtio%ld creation size %x, paddr=%llx, irq: %d", idx, vi_data->fact->size, (unsigned long long)paddr, vi_data->fact->vector);
	...
	section_sz = vi_data->fact->size * 0x1000 - ((section_addr - (GUEST_SHM_PADDR + (idx << 32)) - 0x1000 + (min_align - 1))  & ~(min_align - 1));
	...
	vi_dev->shmem_sz_used = (vi_dev->virtio_header->size + (chunk_size - 1)) & ~(chunk_size - 1);
